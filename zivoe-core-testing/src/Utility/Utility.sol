...
contract Utility is Test {
    ...
    address[19] actors; // @audit the array of all the actors created in the setup
    ...
    address constant SUSHI_V2_ROUTER =
    ...
    // @audit created the actors array so it can be used for fuzzing actually used accounts
    function createActors() public {
        // 2 Admins.
        god = new Admin();
        zvl = new Admin();
        actors[0] = address(god);
        actors[1] = address(zvl);

        // 1 Blackhat.
        bob = new Blackhat();
        actors[2] = address(bob);

        // 1 Borrower.
        tim = new Borrower();
        actors[3] = address(tim);

        // 1 Deployer.
        jay = new Deployer();
        actors[4] = address(jay);

        // 1 Manager.
        roy = new Manager();
        actors[5] = address(roy);
        ...
        jon = new Investor();
        jen = new Investor();
        actors[6] = address(sam);
        actors[7] = address(sue);
        actors[8] = address(sal);
        actors[9] = address(sid);
        actors[10] = address(jim);
        actors[11] = address(joe);
        actors[12] = address(jon);
        actors[13] = address(jen);
        ...
        pam = new Vester();
        tia = new Vester();
        actors[14] = address(poe);
        actors[15] = address(qcp);
        actors[16] = address(moe);
        actors[17] = address(pam);
        actors[18] = address(tia);
    }
        ...
        if (stake) {
            assert(
                sam.try_approveToken(
                    address(zSTT),
                    address(stSTT),
                    IERC20(address(zSTT)).balanceOf(address(sam))
                )
            );
            assert(
                sam.try_stake(
            ...
            assert(
                jim.try_approveToken(
                    address(zJTT),
                    address(stJTT),
                    IERC20(address(zJTT)).balanceOf(address(jim))
                )
            );
            assert(
                jim.try_stake(
    ...
    function stakeTokens() public {
        ...
        assert(
            sue.try_approveToken(
                address(ZVE),
                address(stZVE),
                IERC20(address(ZVE)).balanceOf(address(sue))
            )
        );
        assert(
            sal.try_approveToken(
                address(ZVE),
                address(stZVE),
                IERC20(address(ZVE)).balanceOf(address(sal))
            )
        );
        assert(
            sid.try_approveToken(
                address(ZVE),
                address(stZVE),
                IERC20(address(ZVE)).balanceOf(address(sid))
            )
        );
        assert(
            jim.try_approveToken(
                address(ZVE),
                address(stZVE),
                IERC20(address(ZVE)).balanceOf(address(jim))
            )
        );
        assert(
            joe.try_approveToken(
                address(ZVE),
                address(stZVE),
                IERC20(address(ZVE)).balanceOf(address(joe))
            )
        );
        assert(
            jon.try_approveToken(
                address(ZVE),
                address(stZVE),
                IERC20(address(ZVE)).balanceOf(address(jon))
            )
        );
        assert(
            jen.try_approveToken(
                address(ZVE),
                address(stZVE),
                IERC20(address(ZVE)).balanceOf(address(jen))
            )
        );
        ...
        assert(
            sue.try_approveToken(
                address(zSTT),
                address(stSTT),
                IERC20(address(zSTT)).balanceOf(address(sue))
            )
        );
        assert(
            sal.try_approveToken(
                address(zSTT),
                address(stSTT),
                IERC20(address(zSTT)).balanceOf(address(sal))
            )
        );
        assert(
            sid.try_approveToken(
                address(zSTT),
                address(stSTT),
                IERC20(address(zSTT)).balanceOf(address(sid))
            )
        );
        assert(
            jim.try_approveToken(
                address(zSTT),
                address(stSTT),
                IERC20(address(zSTT)).balanceOf(address(jim))
            )
        );
        assert(
            joe.try_approveToken(
                address(zSTT),
                address(stSTT),
                IERC20(address(zSTT)).balanceOf(address(joe))
            )
        );
        assert(
            jon.try_approveToken(
                address(zSTT),
                address(stSTT),
                IERC20(address(zSTT)).balanceOf(address(jon))
            )
        );
        assert(
            jen.try_approveToken(
                address(zSTT),
                address(stSTT),
                IERC20(address(zSTT)).balanceOf(address(jen))
            )
        );
        ...
        assert(
            sam.try_approveToken(
                address(zJTT),
                address(stJTT),
                IERC20(address(zJTT)).balanceOf(address(sam))
            )
        );
        assert(
            sue.try_approveToken(
                address(zJTT),
                address(stJTT),
                IERC20(address(zJTT)).balanceOf(address(sue))
            )
        );
        assert(
            sal.try_approveToken(
                address(zJTT),
                address(stJTT),
                IERC20(address(zJTT)).balanceOf(address(sal))
            )
        );
        assert(
            sid.try_approveToken(
                address(zJTT),
                address(stJTT),
                IERC20(address(zJTT)).balanceOf(address(sid))
            )
        );
        assert(
            jim.try_approveToken(
        ...
        assert(
            joe.try_approveToken(
                address(zJTT),
                address(stJTT),
                IERC20(address(zJTT)).balanceOf(address(joe))
            )
        );
        assert(
            jon.try_approveToken(
                address(zJTT),
                address(stJTT),
                IERC20(address(zJTT)).balanceOf(address(jon))
            )
        );
        assert(
            jen.try_approveToken(
                address(zJTT),
                address(stJTT),
                IERC20(address(zJTT)).balanceOf(address(jen))
            )
        );
    ...
    function deployCore(bool live) public {
        ...
        // @audit creates multiple contract accounts that have designated purposes
        createActors();
        // @audit set token variables in tokens mapping
        setUpTokens();
        ...
        // @audit calling contract becomes owner of TLC contract
        TLC = new ZivoeTLC(12 hours, proposers, executors, address(GBL));

        GOV = new ZivoeGovernorV2(IVotes(address(ZVE)), TLC, address(GBL));

        // TLC.owner() MUST grant "EXECUTOR_ROLE" to address(0) for public execution of proposals.
        // @audit seems dangerous to force granting to a default address
        ...
        zSTT = new ZivoeTrancheToken("Zivoe Senior Tranche", "zSTT");

        zJTT = new ZivoeTrancheToken("Zivoe Junior Tranche", "zJTT");
        ...
        ITO = new ZivoeITO(address(GBL), _stablesITO);
        ...
        // @audit this makes owner privileged functions in the TrancheToken contracts unusable
        zJTT.renounceOwnership();
        zSTT.renounceOwnership();

        // Step #8 --- Deploy zSTT/zJTT/ZVE staking contracts, through ZivoeRewards.sol.

        stSTT = new ZivoeRewards(address(zSTT), address(GBL));

        stJTT = new ZivoeRewards(address(zJTT), address(GBL));

        stZVE = new ZivoeRewards(address(ZVE), address(GBL));

        // Step #9 --- Deploy ZivoeYDL.sol.

        YDL = new ZivoeYDL(address(GBL), DAI);

        MATH = YDL.MATH();

        // Step #10 --- Deploy ZivoeRewardsVesting.sol.

        vestZVE = new ZivoeRewardsVesting(address(ZVE), address(GBL));

        // "jay" MUST transfer 60% of ZVE tokens to vestZVE.
        jay.transferToken(
            address(ZVE),
            address(vestZVE),
            (ZVE.totalSupply() * 6) / 10
        );
        ...
        // @audit startPrank will cause issues with crytic, replacing with prank instead
        ...
        // @audit will mess up assertion testing, refactoring as require instead
        // assert(zvl.try_updateIsLocker(address(GBL), address(ZVT), true));
        require(zvl.try_updateIsLocker(address(GBL), address(ZVT), true));
    ...
    function stakeTokensHalf() public {
        ...
        jim.try_stake(
            address(stJTT),
            IERC20(address(zJTT)).balanceOf(address(jim)) / 2
        );
        ...
        sam.try_stake(
            address(stSTT),
            IERC20(address(zSTT)).balanceOf(address(sam)) / 2
        );
    ...
    function simulateDepositsCoreUtility(
        ...
        assert(
            sam.try_approveToken(DAI, address(ITO), seniorDeposit * 1 ether)
        );
        ...
        assert(
            sam.try_depositSenior(
                address(ITO),
                seniorDeposit * 1 ether,
                address(DAI)
            )
        );
        assert(
        ...
        assert(
            jim.try_approveToken(
                DAI,
                address(ITO),
                (seniorDeposit * 1 ether) / 5
            )
        );
        assert(
            jim.try_approveToken(USDC, address(ITO), (seniorDeposit * USD) / 5)
        );
        assert(
            jim.try_approveToken(USDT, address(ITO), (seniorDeposit * USD) / 5)
        );

        assert(
            jim.try_depositJunior(
                address(ITO),
                (seniorDeposit * 1 ether) / 5,
                address(DAI)
            )
    ...
    function mint(bytes32 symbol, address account, uint256 amount) public {
        ...
        // @audit replacing this with a require to not have echidna check it
        // assertEq(IERC20(addr).balanceOf(account), bal + amount); // Assert new balance
        require(IERC20(addr).balanceOf(account) == bal + amount);
    }
